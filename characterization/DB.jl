...
* `lanes` - Number of lanes for that app - generated by the Project Manager.
* `architecture` - Architecture the application was mapped to to generate the
    data
* `time` - Time the data was added to the database.
* `note` - Any note regarding the data.
"""
