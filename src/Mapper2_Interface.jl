...
function Mapper2.ismappable(::KC, c::Component)
...
function Mapper2.isspecial(::KC, t::TN)
...
# with components.
function Mapper2.isequivalent(::KC, a::TN, b::TN)
...
function Mapper2.canmap(::KC, t::TN, c::Component)
    ismappable(c) || return false
    return in(t.metadata[typekey()], c.metadata[typekey()])
end

function Mapper2.is_source_port(::KC, p::Port, e::TE)
...
function Mapper2.is_sink_port(::KC, p::Port, e::TE)
    port_link_class = p.metadata["link_class"]
    edge_link_class = e.metadata["link_class"]
...
function Mapper2.needsrouting(::KC, edge::TaskgraphEdge)
...
function Mapper2.SA.build_channels(::KC, channel, sources, sinks)
...
function Mapper2.routing_channel(::KC, start, stop, edge)
